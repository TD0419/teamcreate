...
void CObjTitle::Draw()
{
	...
	if( m_mode == 0 )
	{
		Font::StrDraw(L"Start"	, WINDOW_SIZE_W - 250.0f, WINDOW_SIZE_H/2.0f + 40.0f    , CHAR_SIZE , color_yellow);
		Font::StrDraw(L"Option"	, WINDOW_SIZE_W - 250.0f, WINDOW_SIZE_H/2.0f + 118.0f	, CHAR_SIZE - 10.0f, color_white);
		Font::StrDraw(L"Exit"	, WINDOW_SIZE_W - 250.0f, WINDOW_SIZE_H/2.0f + 198.0f	, CHAR_SIZE - 10.0f, color_white);

	}
	//→がOptionを指している場合は、Optionの文字を黄色にする。StartとExitの文字は白色。
	else if ( m_mode == 1 )
	{
		Font::StrDraw(L"Start", WINDOW_SIZE_W - 250.0f, WINDOW_SIZE_H / 2.0f + 40.0f, CHAR_SIZE - 10.0f, color_white);
		Font::StrDraw(L"Option", WINDOW_SIZE_W - 250.0f, WINDOW_SIZE_H / 2.0f + 118.0f, CHAR_SIZE , color_yellow);
		Font::StrDraw(L"Exit", WINDOW_SIZE_W - 250.0f, WINDOW_SIZE_H / 2.0f + 198.0f, CHAR_SIZE - 10.0f, color_white);
	}
	...
	else if ( m_mode == 2 )
	{
		Font::StrDraw(L"Start", WINDOW_SIZE_W - 250.0f, WINDOW_SIZE_H / 2.0f + 40.0f, CHAR_SIZE -10.0f, color_white);
		Font::StrDraw(L"Option", WINDOW_SIZE_W - 250.0f, WINDOW_SIZE_H / 2.0f + 118.0f, CHAR_SIZE -10.0f, color_white);
		Font::StrDraw(L"Exit", WINDOW_SIZE_W - 250.0f, WINDOW_SIZE_H / 2.0f + 198.0f, CHAR_SIZE , color_yellow);
	}

	...
	for (int i = 0; i < hit->GetCount(); i++)
	{
		...
		if (hit_data[i] != nullptr)
		{
			...
			if (0 < r && r < 45 || 315 < r && r < 360)
			{
				...
				objboss->SetPosX(m_px + BLOCK_SIZE);//ボスの位置をブロックの右側までずらす
			}
			//ブロックの上側が衝突している場合
			else if (45 < r && r < 125)
			{
				objboss->SetVecY(0.0f);//ボスのY方向の移動を０にする
				objboss->SetPosY(m_py - BOSS_SIZE);//ボスの位置をブロックの上側までずらす
			}
			//ブロックの左側が衝突している場合
			else if (125 < r && r < 225)
			{
				objboss->SetVecX(0.0f);//ボスのX方向の移動を０にする
				objboss->SetPosX(m_px - BOSS_SIZE);//ボスの位置をブロックの左側までずらす
			}
			//ブロックの下側が衝突している場合
			else if (225 < r && r < 315)
			{
				objboss->SetVecY(0.0f);//ボスのY方向の移動を０にする
				objboss->SetPosY(m_py + BLOCK_SIZE);//ボスの位置をブロックの上側までずらす
			}
		}
	}
}